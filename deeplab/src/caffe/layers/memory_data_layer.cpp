...
void MemoryDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  top[2]->Reshape(batch_size_, 1, 1, 2);
  added_data_.Reshape(batch_size_, channels_, height_, width_);
  added_label_.Reshape(label_shape);
  added_datadim_.Reshape(batch_size_, 1, 1, 2);
  data_ = NULL;
  labels_ = NULL;
  datadim_ = NULL;
  added_data_.cpu_data();
  added_label_.cpu_data();
  added_datadim_.cpu_data();
}
...
void MemoryDataLayer<Dtype>::AddDatumVector(const vector<Datum>& datum_vector) {
  ...
  Reset(top_data, top_label, NULL, num);
  has_new_data_ = true;
}
...
void MemoryDataLayer<Dtype>::AddMatVector(const vector<cv::Mat>& mat_vector,
  ...
  added_data_.Reshape(num, channels_, height_, width_);
  added_label_.Reshape(num, 1, 1, 1);
  added_datadim_.Reshape(num, 1, 1, 2);
  // Apply data transformations (mirror, scale, crop...)
...
template <typename Dtype>
void MemoryDataLayer<Dtype>::Reset(Dtype* data, Dtype* labels, Dtype* datadims,int n) {
  CHECK(data);
  CHECK(labels);
  CHECK(datadims);
...
void MemoryDataLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  top[2]->set_cpu_data(datadim_ + pos_ * 2);
